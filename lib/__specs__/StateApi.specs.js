...
context('#StateApi specs', () => {
  ...
  describe('When adding comments', () => {
    const comment = {
      username: 'someone', 
      body: 'I really like this comment',
      articleId: 1
    };

    const addedComment = api.addComment(comment);

    it('should equal to the provided username', () => addedComment.username.should.be.equal(comment.username));
    it('should have an id', () => addedComment.id.should.not.be.null);
  });
});
